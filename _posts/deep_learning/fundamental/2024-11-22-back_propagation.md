...
&nbsp; 심층 신경망(Deep Neural Network, DNN)의 목표는 실제 값과 출력 값 간의 오차를 최소화하는 것입니다.<br>
이를 위해 경사하강법(Gradient Descent)을 사용하여 파라미터를 반복적으로 업데이트하며, 연쇄법칙(Chain rule)을 활용하여 파라미터를 효율적으로 업데이트하는 오차 역전파(Back propagation)가 활용됩니다.